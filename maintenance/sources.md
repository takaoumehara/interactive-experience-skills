...
| 権利 | JASRAC / NexTone の利用料規程、各国の管理団体 | 演奏権・シンクロ権の**手続きの構造と免除条件**。⚠️ **料率は機械取得不可** — 下記 |
| 素材 | IR（インパルス応答）素材の配布元とライセンス | 商用可否。OpenAIR（ヨーク大学、CC系）が第一候補 |

**⚠️ 機械取得不可: `jasrac.or.jp`**（2026-07 確認。HTML・PDF とも 403）。
**毎月の再試行の対象から外す。**料率の数値は自動更新では埋まらない。
二次情報の数値で代用せず、手続きの構造（窓口の分かれ方、免除の3条件、
作詞作曲権と原盤権が別枠であること）だけを維持する。
料率が必要な案件では**管轄支部への問い合わせ工程を見積に入れる**、という
指針の側で解く。
