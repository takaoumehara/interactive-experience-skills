...
### ③ 새로운 수단 탐색 (분기에 한 번 정도)

```
/scout-skills
```

`/refresh-skills`는 **이미 쓰여 있는 것이 지금도 맞는지**를 확인하는 명령이고, 새로운 기술을 발견해도 임의로 추가하지 않습니다. 그래서 새로운 수단은 구조상 그쪽으로는 들어오지 않습니다.

그것을 줍는 것이 `/scout-skills`입니다. 묘사·음향·취득·배포 네 영역을 조사하고, **레퍼런스는 전혀 고치지 않은 채** `CANDIDATES.md`에 후보로 추가합니다. 채택 여부는 당신이 정합니다.

후보가 되려면 다음 네 가지를 모두 통과해야 합니다.

1. 기존 수단으로는 낼 수 없는 표현·판단을 낼 수 있는가
2. 개인·소규모에서 도달 가능한가
3. **어떻게 망가지는지 쓸 수 있는가**
4. **어느 기존 기술에 연결되거나 그것을 대체하는지** 말할 수 있는가

4에서 떨어지는 것이 가장 많고, 그래도 됩니다. 레퍼런스가 비대해지면 skill은 읽는 양을 줄이기 시작하고, **두껍게 만든 탓에 품질이 떨어집니다.** 이 명령의 가치는 추가한 수가 아니라 통과시키지 않는 데 있습니다.

월 단위로 돌리지 마십시오. 이 분야는 한 달로는 의미 있는 변화가 없고, "후보 없음"을 흘려보내는 습관이 들면 정작 바뀌었을 때 놓칩니다.
...
②의 검증과 ③의 탐색을 **순서대로** 한 번에 돌립니다. 순서에 의미가 있습니다. 새로운 것을 찾기 전에 지금 쓰여 있는 것이 맞는지를 확정하지 않으면, 낡은 기술 위에 새 기술을 쌓게 되기 때문입니다.

결과는 두 개의 보고가 아니라 통합된 한 장의 표로 나옵니다. 반영은 반드시 물어본 뒤에 하고, 실행 로그에 한 줄을 남깁니다.

```
/skills-routine 음향     # 탐색을 음향으로만 좁힘
/skills-routine verify   # 검증만 하고 멈춤
```

**절차·트리거·실행 로그는 [`ROUTINE.md`](ROUTINE.md)에 있습니다.**

의도적으로 스케줄러에 올리지 않았습니다. cron은 머신이 바뀌면 사라지고, 반복 알림은 세 번째부터 읽히지 않습니다. 대신 `ROUTINE.md`가 가진 것은 **트리거 목록**입니다.

| 트리거 | 실행할 것 |
|---|---|
| Apple 플랫폼 발표 | `/scout-skills 描画` `/scout-skills 配布` |
| 브라우저의 GPU / 미디어 지원이 움직임 | `/scout-skills 描画` `/scout-skills 音響` |
| **skill의 답변에 낡은 전제가 섞여 있었다** | `/refresh-skills` |
| **실제 프로젝트에서 「이건 레퍼런스에 없네」라고 느꼈다** | 그 자리에서 `CANDIDATES.md`에 직접 적기 |

**아래 두 개가 가장 가치가 높습니다.** 실제로 쓰면서 느낀 결락은 웹 검색으로는 절대 찾을 수 없습니다. 날짜가 아니라 이것이 진짜 신호입니다.
